...
struct TargetFunctionInfo {
  // No signature mismatch between call site and device function prototype.
  ...
  // Potential signature mismatch - generate conversions if needed.
  TargetFunctionInfo(const string callee_name_, bool check_signature_,
  ...
  // Potential signature mistamatch - use bitcast if specified by the target.
  TargetFunctionInfo(const string callee_name_, bool check_signature_,
                     absl::Span<const PrimitiveType> input_types_,
...
// Note: Only intrinsic or device function - not both - is expected.
struct TargetInfo {
  ...
  TargetInfo(struct TargetIntrinsicInfo x) : target_intrinsic_info(x) {}
...
// The call site provides arguments that is least restrictive to both targets.
// For instance, if one target provides device function that takes 2 arguments
// and the other provides arguments that takes 4 arguments, then the call
// site provides four arguments. The following function  takes care of ignoring
// the two superfluous argumenrs for the GPU device function that takes 2
// arguments. In a similar vein, the following function also specifies
// if conversions need to be generated for any argument type mismatches.
// See check_signature  and use_bitcast fields in TargetFunctionInfo
  ...
  switch (function_id) {
    ...
    case TargetFunctionID::kShflDownI32: {
      ...
      TargetFunctionInfo amdgpu_function_info(
          "__ockl_readuplane", true,
          {PRIMITIVE_TYPE_INVALID, S32, S32, PRIMITIVE_TYPE_INVALID}, S32,
           /* Generate conversions for AMDGN */ true);
      if (target_triple.getArch() == llvm::Triple::nvptx ||
          target_triple.getArch() == llvm::Triple::nvptx64) {
        return TargetInfo(nvptx_intrinsic_info);
      } else {
    ...
    case TargetFunctionID::kThreadIdx: {
      ...
      TargetIntrinsicInfo amdgpu_intrinsic_info(
...
llvm::Value* EmitCallToTargetFunction(
  ...
  if (gpu_info.target_intrinsic_info) {
    ...
    // GPU specifies a signature to be checked.
